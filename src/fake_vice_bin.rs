			...
			match response_type {
				0x31 => {	// registers get
					...
					for e in 0..count {
						...
						let value = (( buffer[ entry_start+3 ] as u16 ) << 8 ) | ( buffer[ entry_start+2 ] as u16 );

						let r = self.registers.entry( id ).or_insert_with(|| Register::default());
						println!("{:#02} | {:#04x} {:#04x} {:#06x} | {}", e, size, id, value, r.name());
						r.set_value( value );
						entry_start += 4;
					}
				},
...
byte 4+: Name 
    				*/
    				...
    				for e in 0..count {
						...
						println!("{:#02} | {:#04x} {:#04x} {:#04x} -> {}", e, size, id, r_size, name);
						let r = self.registers.entry( id ).or_insert_with(|| Register::default());
						r.set_name( name );
						r.set_size( r_size );
						entry_start += size + 1;
				...
				o => {
					match error_code {
						...
						ec => {
							println!("Unhandled response type {:#04x} (error code: {:#04x})", o, ec);
						},
					}
				},
			}
